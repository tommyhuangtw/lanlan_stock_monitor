...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get all active sources - all users receive the same sources now
    ...
    for (const user of users) {
      ...
      // paid = daily, free trial (< 3 days) = daily, free after trial = Monday only (same daily digest)
      ...
      try {
        // Get cached digest - all users use the same sources
        ...
        if (!cacheResult.found) {
          if (cacheResult.reason === 'no_content') {
            // No new content for this combination
            results.skipped++;
            continue;
          }

          // Cache miss - digest not generated yet
          results.cacheMisses++;
          results.errors.push(
        ...
        // Generate unsubscribe URL using the same magic link token
        ...
        // Send email with List-Unsubscribe header
